...
//				}
			...
			if(params.length > 1) {
				...
				if("GET".equals(request.getMethod())){
					for(int i = 0; i < params.length; i++) {
						...
						params[i] = new String(b, StandardCharsets.UTF_8);
					}
				}
				...
				if("GET".equals(request.getMethod())){
					...
					param = new String(b, StandardCharsets.UTF_8);
				} else {
					param = params[0];
				}
	...
	public static Map<String, Object> getParameterMap(MultipartHttpServletRequest req) throws Exception {
		return getParameterMap((HttpServletRequest) req);
	}
